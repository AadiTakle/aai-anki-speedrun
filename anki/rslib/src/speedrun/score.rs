...
//!   [`Collection::topic_mastery`] response (at least one existing mapped card).
//! - **coverage_pct**: `100 * (Σ blueprint_weight over covered topics) /
//!   (Σ blueprint_weight over ALL taxonomy topics)`; `0.0` when the total
//!   taxonomy weight is `0`.
//! - **point**: `100 * (Σ blueprint_weight*avg_recall over covered topics) /
//!   (Σ blueprint_weight over covered topics)`, clamped to `[0, 100]`; when the
...
//!   already caught by this rule. When abstained, `point/low/high` are all `0.0`
//!   (the real `coverage_pct` is still reported) and `reasons` names, with
//!   numbers, which condition failed. Otherwise a non-degenerate band with
//!   `0 <= low <= point <= high <= 100` and `high > low` is produced.
//! - **band**: the uncertainty half-width shrinks with `sqrt(n)` and with higher
//!   coverage, so it is monotonically non-increasing in `n` (more graded
...
impl Collection {
    /// Compute the blueprint-weighted memory score with its uncertainty band and
    /// the abstain rule. See the module docs for the exact definitions. This is
    /// a read-only query: it never mutates the collection.
...
mod test {
    ...
    /// 3. Enough reviews AND enough coverage -> score, with a valid non-degenerate
    ///    band and a fresh timestamp.
    #[test]
    ...
    /// 4. Coverage weighting: two equal-weight topics with only one covered gives
    ///    ~50% coverage; and higher average recall yields a higher point.
    #[test]
    ...
    /// 5. Read-only + deterministic: two calls on fixed data agree, no undo step
    ///    is created, and the database stays uncorrupted.
    #[test]
