...
export interface PointsAtStakeTopicView {
    topicId: string;
    name: string;
    /** Topic's share of the exam blueprint on a 0..100 scale (the adapter scales
     * the backend's 0..1 fraction), e.g. Cardiovascular ~11. */
    blueprintWeight: number;
    /** 0.0 = strong, 1.0 = weak. */
    weakness: number;
    /** blueprintWeight (0..100) * weakness — the ranking key (higher = study
     * first), so a top topic reads ~5–11 "pts", not ~0.05. */
    points: number;
}
...
export interface CoverageSectionView {
    ...
    name: string;
    coveragePct: number;
    /** Share of the exam blueprint on a 0..100 scale (adapter-scaled from the
     * backend's 0..1 fraction), consistent with points-at-stake. */
    blueprintWeight: number;
}
...
/** One task in the daily-loop to-do (mirrors anki.speedrun.DailyTask). */
export type DailyTaskState = "upcoming" | "current" | "done";

export interface DailyTaskView {
    /** Stable id: "plan" | "qblock" | "review" | "close". */
    id: string;
    label: string;
    /** Derived detail line, e.g. "imported today" or "18 / 40 done". */
    detail: string;
    state: DailyTaskState;
    /** Live progress numerator/denominator (review task only; 0 otherwise). */
    doneCount: number;
    totalCount: number;
}

/**
 * The daily loop rendered as a progressing to-do list — the merged "what to do
 * next" surface (anki.speedrun.DailyPlanResponse). Exactly one incomplete task
 * is `current`; when none is, the loop is closed for the day.
 */
export interface DailyPlanView {
    /** In loop order: plan, qblock, review, close. */
    tasks: DailyTaskView[];
}
